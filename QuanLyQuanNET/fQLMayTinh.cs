...
{
    public partial class fQLMayTinh : Form
    {
        int FlagCT = 0;
        //0 là không
        //1 là thêm
        ...
        public fQLMayTinh(string username, string password)
        {
        ...
        private void fQLMayTinh_Load(object sender, EventArgs e)
        {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                txtTenMayTinh.ReadOnly = false;
                txtTrangThaiMT.ReadOnly = false;
                txtThoiGianMo.ReadOnly= true;
                txtMaTaiKhoan.ReadOnly = false;

                txtMaMayTinh.Text = string.Empty;
                txtMaPhong.Text = string.Empty;
                txtTenMayTinh.Text = string.Empty;
                txtTrangThaiMT.Text = string.Empty;
                txtThoiGianMo.Text = string.Empty;
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (FlagCT == 1)
            {
                try
                {
                    SqlCommand command = new SqlCommand();
                    command.CommandType = CommandType.StoredProcedure;
                    command.CommandText = "ThemMayTinh";
                    command.Connection = conn;

                    command.Parameters.Add("@MaPhong", SqlDbType.NVarChar).Value = txtMaPhong.Text;
                    command.Parameters.Add("@TenMayTinh", SqlDbType.NVarChar).Value = txtTenMayTinh.Text;
                    command.Parameters.Add("@TrangThaiMayTinh", SqlDbType.Int).Value = int.Parse(txtTrangThaiMT.Text);
            ...
            if (FlagCT == 2)
            {
                try
                {
                    SqlCommand command = new SqlCommand();
                    command.CommandType = CommandType.StoredProcedure;
                    command.CommandText = "SuaMayTinh";
                    command.Connection = conn;
